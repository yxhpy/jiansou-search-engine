...
class SearchEngineService:
    ...
    @staticmethod
    def get_search_engines(db: Session, active_only: bool = True) -> List[SearchEngine]:
        """获取搜索引擎列表"""
    ...
    @staticmethod
    def get_search_engine_by_id(db: Session, engine_id: int) -> Optional[SearchEngine]:
        """根据ID获取搜索引擎"""
        return db.query(SearchEngine).filter(SearchEngine.id == engine_id).first()
    
    @staticmethod
    def get_search_engine_by_name(db: Session, name: str) -> Optional[SearchEngine]:
        """根据名称获取搜索引擎"""
        return db.query(SearchEngine).filter(SearchEngine.name == name).first()
    
    @staticmethod
    def create_search_engine(db: Session, engine_data: SearchEngineCreate) -> Optional[SearchEngine]:
        """创建搜索引擎"""
        # 检查名称是否已存在
        existing = SearchEngineService.get_search_engine_by_name(db, engine_data.name)
        if existing:
            return None
        
        # 如果设置为默认搜索引擎，取消其他的默认设置
        if engine_data.is_default:
            SearchEngineService._clear_default_engines(db)
    ...
    @staticmethod
    def update_search_engine(db: Session, engine_id: int, engine_data: SearchEngineUpdate) -> Optional[SearchEngine]:
        """更新搜索引擎"""
        db_engine = SearchEngineService.get_search_engine_by_id(db, engine_id)
        if not db_engine:
            return None
    ...
    @staticmethod
    def delete_search_engine(db: Session, engine_id: int) -> bool:
        """删除搜索引擎"""
        db_engine = SearchEngineService.get_search_engine_by_id(db, engine_id)
        if not db_engine:
            return False
    ...
    @staticmethod
    def get_default_search_engine(db: Session) -> Optional[SearchEngine]:
        """获取默认搜索引擎"""
    ...
    @staticmethod
    def _clear_default_engines(db: Session):
        """清除所有默认搜索引擎设置"""
